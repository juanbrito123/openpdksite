    ...
    <div className="min-h-screen flex flex-col md:flex-row bg-[#f4f5f7]">
      ...
      <aside className="hidden md:flex flex-col w-72 h-screen sticky top-0 bg-white border-r border-slate-200 z-50">
        ...
        <nav className="flex-1 p-4 space-y-2 overflow-y-auto">
          <div className="text-[10px] font-bold text-slate-400 uppercase tracking-widest px-4 mb-2">Projects & Workspace</div>
      ...
      <main className="flex-1 flex flex-col min-h-screen relative overflow-x-hidden technical-grid">
        <div className="flex-1 p-6 md:p-10 max-w-7xl w-full mx-auto">
          <div className="flex items-center gap-2 text-xs text-slate-400 mb-8 px-1">
            ...
            <span>/</span>
            <span className="text-slate-600 font-bold capitalize bg-slate-200/50 px-2 py-0.5 rounded">{page === 'home' ? 'Overview' : page}</span>
          </div>
        ...
        {/* Footer */}
        <footer className="mt-12 py-10 px-10 bg-white border-t border-slate-200">
          <div className="flex flex-col md:flex-row justify-between items-center gap-6 text-[11px] text-slate-400 font-semibold tracking-wide uppercase">
            <p>&copy; {new Date().getFullYear()} OpenSource IC Brazil. {t.footer.rights}</p>
            <div className="flex gap-8">
              <a href="https://github.com/IHP-GmbH/IHP-Open-PDK" target="_blank" className="hover:text-emerald-600 transition-colors">GitHub Repository</a>
              <a href="https://www.ufrgs.br/cadmicro/ciaberto/" target="_blank" className="hover:text-emerald-600 transition-colors">CI Aberto Initiative</a>
            </div>
          </div>
        </footer>

        {/* Mobile Bottom Nav */}
        <nav className="md:hidden sticky bottom-0 left-0 right-0 bg-white border-t border-slate-200 px-4 py-2 flex justify-around items-center z-50 shadow-[0_-4px_12px_rgba(0,0,0,0.08)]">
...
const LandingPage: React.FC<{ t: any; setPage: (p: Page) => void }> = ({ t, setPage }) => (
  <div className="space-y-10">
    ...
    <div className="grid md:grid-cols-2 gap-8">
      ...
      <div className="bg-emerald-900 p-10 rounded-3xl shadow-2xl flex flex-col justify-center text-white relative overflow-hidden group">
      ...
      <table className="w-full text-left border-collapse min-w-[700px]">
        <thead className="bg-slate-50 border-b border-slate-200">
          <tr>
            ...
            <th className="px-8 py-5 text-[11px] font-black text-slate-400 uppercase tracking-widest w-48 text-right">Actions</th>
          </tr>
        </thead>
